...
Scene 1 (0.0–1.2s): flat `bg-primary` white ground (the contrast turn from Frame 2's black). "Meet the AI lead capture form" TYPES in left-aligned, Raleway body-scale, `text-primary` ink, one `label` eyebrow above it reading "INTRODUCING" in `accent` gold — Asymmetric 70/30, left text / right clear space.
Scene 2 (1.2–2.4s): as "built into every" plays, `assets/logo-icon.svg` (the gold bolt-on-black mark) pops in at the right 30%, small, with a light spring-scale (power3, minimal overshoot per Cartesian's no-bounce rule — read as a clean settle, not a bounce).
...
Reproduce: one specific multi-step workflow (typing through the form's real qualifying fields) shown end-to-end across 4 discrete beats, each a real edit the surface answers live, camera chasing the custom cursor, landing locked on the final selected dropdown option.
...
Scene 3 (2.2–3.2s): on "$497 Coaching Landing Page," `assets/logo-icon.svg` spring-blooms from ZERO at dead center (power3, minimal overshoot) then slides left as "$497 · Coaching Landing Page" reveals to its right in `label`-scale gold uppercase tracking.
Scene 4 (3.2–4.5s): on "Live in 48 hours," a small `attribution`-scale line settles beneath the lockup reading "LIVE IN 48 HOURS." The whole lockup holds dead static — the video's longest hold — to the end of the clip.
