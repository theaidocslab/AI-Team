...
description: >
  ...
  identical and sacred — the five-tone warm-stone palette, Playfair Display 400 + Raleway, the
...
typography:
  # — reading ramp (Raleway) —
  body:        { fontFamily: "Raleway", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "Raleway", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "Raleway", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "Raleway", px: 14, weight: 500, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "Raleway", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "Raleway", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
...
- **Fonts** — already set to **Playfair Display** (display) / **Raleway** (body); ignore any preset font name lingering in prose.
...
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **Playfair Display**
...
- **Playfair Display 400** (ink, sentence case) for display; **Raleway** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (Raleway body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **Raleway labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs Raleway sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an Playfair Display subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **Playfair Display + Raleway via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
